...
The plot below shows Ar statistics calculated by-reef. This clearly shows that values from Adjacent reefs (with the exception of West Pelorus) have consistently higher values than bays at Magnetic Island. This is notable because Ar declines rapidly in response to a reduction in population size.
...
Permutation tests were also used to calculate significance of differences for these statistics.  This is done on aggregated data, grouped into ADJ, NO and SO.  Note that Fis is always higher on aggregated data which suggests a Wahlund effect even though we have avoided lumping sampled from genetically differentiated populations.  
