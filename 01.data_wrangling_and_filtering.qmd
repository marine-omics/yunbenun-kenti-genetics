...
```{r}
dartR.base::gl.report.callrate(ak, method = "ind", ind.to.list = 528)
```

# Duplicates

16 samples were deliberately duplicated in addition to any technical replicates provided by DaRT. 

```{r,eval=FALSE}
ab_pairs <- read_tsv("raw_data/dup_pairs.tsv",col_names = c("A","B"))
```

Using the dartR `gl.report.replicates` function we see that all 16 are identified as having extremely high concordance but the picture is slightly messy because there are also clusters of related individuals.  We retain all individuals for now and will remove clones/replicates in the next step.

```{r,eval=FALSE}
rep_res <- gl.report.replicates(ak,plot.out = FALSE)

dup_samples <- c(ab_pairs$A,ab_pairs$B)

dup_matches <- rep_res$table.rep |>
  filter(ind1 %in% dup_samples & ind2 %in% dup_samples) |>
  arrange(desc(perc))
```


For now we note that, of our 16 known pairs the following are flagged for removal because they had the lower callrate of the pair.

> 'At_162_J20.1', 'At_163_J20.1', 'B1', 'A2', 'A3', 'A7', 'B8', 'A9', 'A10', 'B11', 'A15', 'B17', 'B18', 'A19', 'A21', 'WB_03/04/2022_36'

These duplicates come from South Orpheus (15 samples) and Wilson Bay (1 sample).
...
save(ak.filtered, file="cache/ak_r1.filtered.rdata")
```
...
```{r}
write.table(ak.filtered$loc.names,file = "dart2vcf/ak_r1.filtered.loci",row.names = FALSE,quote = FALSE)
write.table(ak.filtered$ind.names,file = "dart2vcf/ak_r1.filtered.ind",row.names = FALSE,quote = FALSE,col.names = FALSE)
```
...
This process results in a loss of around 10% of loci (49989 of 55247), mostly because they cannot be mapped to the genome. Thus the vcf file will have slighly fewer loci than the genlight object used by dartR.
...
# This filter for individuals doesn't introduce any monomorphic loci so it stays the same at 4942
