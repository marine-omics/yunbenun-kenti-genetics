...
To determine the appropriate value of K we plot K vs the likelihood

```{r}
k_vs_lik <- read_table("structure/k_vs_lik.txt",col_names = c("k","logLik"))
ggplot(k_vs_lik,aes(x=k,y=logLik)) + geom_point() + geom_line()
```

Although the highest likelihood is achieved for a model with K=3 this is only marginally better than K=2.  This also matches results seen in PCA where we can see a very clear separation between two populations (see below).  In such situations the structure manual (and occam's razor) suggests that the smaller K is more appropriate, especially since mild deviations from the model assumed by structure tends to lead to an overestimation of K.
...
run12 <-  read_table("structure/ak.structure.nr.k2.out.anc.txt",col_names = c("X1", "Label","(%Miss)","C1","C2"),skip = 2) #%>% 
  select(ID=Label,C1,C2) %>% # Select only useful columns
...
```{r}
ggplot(run12_meta, aes(x = ID, y = p, fill=cluster)) +
