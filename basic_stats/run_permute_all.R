...
data_matched <- read_rds("hfstat_nokin.rds")
...
#     observed value.
#
#  2. Taking the minimum across this dataset and the kin-removed dataset puts
#     both permutation runs on the same depth, so their p-values are directly
#     comparable. Dropping individuals can only lower the per-locus counts, so
#     this minimum always comes from the kin-removed data (34 and 72 alleles
#     for the no/so/ADJ and MI/ADJ groupings respectively).
...
# offset differs from run_permute_nokin.R so the two nulls stay independent.
set.seed(3271 + n)
...
write_rds(out,file = paste("perms_all/perm.",n,".rds",sep="",collapse=""))
